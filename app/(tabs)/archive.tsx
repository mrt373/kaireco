...
export default function ArchiveScreen() {
  ...
  return (
    <View className="flex-1 bg-background px-4 pt-4">
      <View className="mb-5">
        <Text className="text-text-primary text-2xl font-bold mb-1">{t("archive.title")}</Text>
        <Text className="text-text-secondary text-sm">{t("archive.subtitle")}</Text>
      </View>
      ...
      <View className="bg-surface border border-border rounded-xl p-4 mb-5">
        ...
        <Text className="text-text-primary text-4xl font-bold mb-1">{MOCK_ARCHIVED.length}</Text>
        <Text className="text-text-muted text-xs mb-4">{t("archive.transformations")}</Text>
        ...
        {METHOD_FILTERS.map((m) => (
          <Pressable
            ...
            className={`px-4 py-2 rounded-full border shrink-0 ${
              selectedMethod === m.value ? "bg-gold border-gold" : "bg-surface border-border"
            }`}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center mt-16">
            <Text className="text-text-muted text-sm">{t("common.noItems")}</Text>
          </View>
        }
      />
    </View>
